...
export function CardFormModal({ isOpen, onClose, onSave, editingCard }: CardFormModalProps) {
  ...
  function onSubmit(data: CardFormValues) {
    const cardData = {
      ...data,
      name: data.title, // Синхронизируем имя с заголовком
    };
  ...
  const handleImageUpload = async (file: File, type: 'card' | 'banner' | 'carousel'): Promise<string | null> => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error || 'Произошла ошибка при загрузке');
      }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <Tabs defaultValue="main" className="w-full">
              ...
              <TabsContent value="main" className="space-y-4 p-4 min-h-[350px]">
              ...
              <TabsContent value="prices" className="space-y-4 p-4 min-h-[350px]">
              ...
              <TabsContent value="images" className="p-4 min-h-[350px]">
            ...
            <DialogFooter className="pt-6 mt-4 border-t border-zinc-700">
